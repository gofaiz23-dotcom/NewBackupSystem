...
const DownloadExcelModal = ({ backendName, tables, onClose }) => {
  ...
  const handleDownload = async () => {
    ...
    try {
      ...
      // Remove backup_ prefix if present
      ...
      const filteredData = allData.map(record => {
        ...
        Object.keys(record).forEach(key => {
          if (key !== 'backup_created_at' && key !== 'backup_updated_at') {
      ...
      XLSX.utils.book_append_sheet(workbook, worksheet, tableNameWithoutPrefix)
      ...
      const filename = `${backendName}_${tableNameWithoutPrefix}_${timestamp}.xlsx`
  ...
  const getTableOptions = () => {
    ...
    return Object.entries(tables.tables)
      ...
      .map(([tableName, tableInfo]) => ({
        value: tableName,
        label: `${tableInfo.originalTableName || tableName.replace('backup_', '')} (${tableInfo.count || 0} records)`,
        ...
        <div className="modal-body">
          ...
          {selectedTable && (
            <div className="download-info">
              ...
              <p>
                <strong>Table:</strong> {selectedTable.replace('backup_', '')}
              </p>
              <p>
